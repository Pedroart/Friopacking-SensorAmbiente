...
namespace crypto_lib
{
  ...
  bool encrypt_ecb_pkcs7(const uint8_t key[16],
                         ...
                         size_t block_size)
  {
    if (!key || !in || !out || !out_len_out)
    {
      Serial.println("E1 null");
      return false;
    }
    if (block_size != 16)
    {
      Serial.println("E2 block");
      return false;
    }
    ...
    {
      Serial.println("E3 cap");
      return false;
    }

    memcpy(out, in, in_len);
    size_t final_len = padding_pkcs7(out, in_len, 16);
    Serial.print("pad final_len=");
    Serial.println(final_len);

    for (size_t off = 0; off < padded_len; off += 16)
    {
      uint8_t enc[16];
      bool ok = aes_ecb_encrypt_sd(key, out + off, enc);
      Serial.print("aes off=");
      Serial.print(off);
      Serial.print(" ok=");
